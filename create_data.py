...
add_arg('annotation_path',      str,  '../IPA-recognition/annotation/', '标注文件的路径')
add_arg('noise_path',           str,  '(不需要)',   '噪声音频存放的文件夹路径')
add_arg('save_audio_path',      str,  '(不需要)',   '合并音频的保存路径')
add_arg('is_change_frame_rate', bool, False,        '是否统一改变音频的采样率')
add_arg('only_keep_zh_en',      bool, False,        '是否只保留中文和英文字符，训练其他语言可以设置为False')
add_arg('max_test_manifest',    int,  100000,       '生成测试数据列表的最大数量，如果annotation_path包含了test.txt，就全部使用test.txt的数据')
